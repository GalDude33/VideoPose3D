...
print(args)
with open(args.checkpoint+'/config.json', 'w') as f:
...
pad = (receptive_field - 1) # Padding on each side
if args.causal:
...
if not args.evaluate:
    ...
    losses_2d_train = []
    losses_2d_train_eval = []
    losses_2d_valid = []
    losses_2d_train_noise = []
    losses_2d_train_eval_noise = []
    losses_2d_valid_noise = []
    ...
    while epoch < args.epochs:
        start_time = time()
        epoch_loss_2d_train = 0
        epoch_loss_2d_train_noise = 0
        N = 0
        ...
        for (_, _, batch_2d) in train_generator.next_epoch():
            ...
            if drop:
                ...
                noise_pred = inputs_2d[:, pad-1:pad, predicted_joint, :2]
            else:
            ...
            loss_2d_pos = mpjpe(predicted_2d_pos+noise_pred, target_2d) # On 2D poses
            loss_2d_pos_noise = mpjpe(noise_pred, target_2d) # On 2D poses
        ...
        losses_2d_train_noise.append(epoch_loss_2d_train_noise / N)
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cam, _, batch_2d in test_generator.next_epoch():
                    ...
                    map2_valid += inputs_2d.shape[0] * inputs_2d.shape[1] * mAP_2(pred2d, target_semi)
                    map2_valid_noise += inputs_2d.shape[0] * inputs_2d.shape[1] * mAP_2(noise_pred, target_semi)
                    map5_valid += inputs_2d.shape[0] * inputs_2d.shape[1] * mAP_5(pred2d, target_semi)
                    map5_valid_noise += inputs_2d.shape[0] * inputs_2d.shape[1] * mAP_5(noise_pred, target_semi)
                    map10_valid += inputs_2d.shape[0] * inputs_2d.shape[1] * mAP_10(pred2d, target_semi)
                    map10_valid_noise += inputs_2d.shape[0] * inputs_2d.shape[1] * mAP_10(noise_pred, target_semi)
                ...
                for cam, _, batch_2d in train_generator_eval.next_epoch():
                    ...
                    # Compute 3D poses
                    predicted_2d_pos = model_pos(inputs_2d)
                    loss_2d_pos = mpjpe(predicted_2d_pos+noise_pred, target_2d)
        ...
        if args.export_training_curves and epoch > 3:
            ...
            plt.plot(epoch_x, losses_2d_train_eval[3:], color='C0')
            plt.plot(epoch_x, losses_2d_valid[3:], color='C1')
            plt.legend(['2d train', '2d train (eval)', '2d valid (eval)'])
            plt.ylabel('MPJPE (m)')
            plt.xlabel('Epoch')
            plt.xlim((3, epoch))
    ...
    with torch.no_grad():
        ...
        for _, _, batch_2d in test_generator.next_epoch():
            ...
            if test_generator.augment_enabled():
                ...
                predicted_2d_pos[1, :, :, 0] *= -1
                predicted_2d_pos[1, :, joints_left + joints_right] = predicted_2d_pos[1, :, joints_right + joints_left]
                predicted_2d_pos = torch.mean(predicted_2d_pos, dim=0, keepdim=True)
                
            if return_predictions:
                return predicted_2d_pos.squeeze(0).cpu().numpy()

            error = mpjpe(predicted_2d_pos+noise_pred, target_2d)
